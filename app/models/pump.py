...
class Pump(Base):
    ...
    prediction_log = relationship(
        "PumpPredictionLog", back_populates="pump", cascade="all, delete-orphan"
    )
...
class PumpPredictionLog(Base):
    ...
    maintenance_required = Column(
        Enum(MaintenanceRequired), nullable=False, default=MaintenanceRequired.no
    )
    confidence_score = Column(Float, nullable=False)
    created_at = Column(DateTime, default=datetime.utcnow)
    pump = relationship("Pump", back_populates="prediction_log")
